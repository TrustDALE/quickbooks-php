...
abstract class QuickBooks_QBXML_Object
{
	 ...
	 * @return self
	 */
	public function set($key, $value, $cast = true): self
	{
		if (is_array($value))
		{
			...
			if (is_array($value))
			{
				...
				foreach ($value as $sub)
				{
					if(is_string($sub) ) {
						...
						$Node->setChildDataAt($root . ' ' . $key, $sub, true);
	...
	static public function fromQBXML($qbxml, $action_or_object = null)
	{
		...
		$tidy = new \tidy();
		$qbxml = $tidy->repairString($qbxml, ['input-xml' => true]);
		...
		if ($Doc = $Parser->parse($errnum, $errmsg))
		{
			...
			if($XML->hasChildren()) {
				...
				foreach($XML->children() as $elem) {
					$response[] = QuickBooks_QBXML_Object::fromXML($elem, $action_or_object)->asArray();
				}
